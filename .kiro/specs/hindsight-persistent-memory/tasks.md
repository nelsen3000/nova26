...
- [ ] 1. Set up core types, configuration, and project structure
  - [ ] 1.1 Create `src/hindsight/types.ts` with MemoryFragment, MemoryFragmentInput, HindsightConfig, FragmentFilter, ScoredFragment, RetrievalContext, ConsolidationReport, HealthStatus, and all supporting interfaces
  ...
  - [ ]* 1.2 Write property test for JSON serialization round-trip
...
- [ ] 2. Implement StorageAdapter interface and SQLiteAdapter
  - [ ] 2.1 Create `src/hindsight/storage-adapter.ts` with the StorageAdapter interface
  ...
  - [ ] 2.2 Create `src/hindsight/sqlite-adapter.ts` implementing StorageAdapter with SQLite + sqlite-vec
  ...
  - [ ]* 2.3 Write property test for export/import round-trip
  ...
  - [ ]* 2.4 Write property test for search filter correctness
...
- [ ] 4. Implement VectorIndex and retrieval scoring
  - [ ] 4.1 Create `src/hindsight/vector-index.ts` implementing the VectorIndex interface
  ...
  - [ ]* 4.2 Write property test for retrieval ranking correctness
...
- [ ] 5. Implement HindsightEngine core
  - [ ] 5.1 Create `src/hindsight/engine.ts` with the HindsightEngine class
  ...
  - [ ]* 5.2 Write property test for fragment creation completeness
...
- [ ] 6. Implement ConsolidationPipeline
  - [ ] 6.1 Create `src/hindsight/consolidation.ts` with the ConsolidationPipeline class
  ...
  - [ ]* 6.2 Write property test for forgetting curve exponential decay
  ...
  - [ ]* 6.3 Write property test for deduplication merges similar fragments
  ...
  - [ ]* 6.4 Write property test for merge preserves highest relevance
  ...
  - [ ]* 6.5 Write property test for below-threshold archived not deleted
  ...
  - [ ]* 6.6 Write property test for consolidation report accuracy
...
- [ ] 8. Implement RetrievalFormatter and pretty-printer
  - [ ] 8.1 Create `src/hindsight/formatter.ts` with RetrievalFormatter and prettyPrint functions
  ...
  - [ ]* 8.2 Write property test for token budget enforcement
  ...
  - [ ]* 8.3 Write property test for type-specific formatting completeness
  ...
  - [ ]* 8.4 Write property test for pretty-printer key fields
...
- [ ] 9. Implement ATLASBridge
  - [ ] 9.1 Create `src/hindsight/atlas-bridge.ts` with the ATLASBridge class
  ...
  - [ ]* 9.2 Write property test for bridge fragment creation
  ...
  - [ ]* 9.3 Write property test for ATLAS tag mapping determinism
...
- [ ] 10. Implement TasteVaultBridge
  - [ ] 10.1 Create `src/hindsight/taste-vault-bridge.ts` with the TasteVaultBridge class
  ...
  - [ ]* 10.2 Write property test for Taste Vault bridge fragment creation
  ...
  - [ ]* 10.3 Write property test for reinforcement boosts relevance
...
- [ ] 11. Implement namespace management and ParallelUniverseBridge
  - [ ] 11.1 Create `src/hindsight/namespace-manager.ts` with namespace fork, merge, and isolation logic
  ...
  - [ ] 11.2 Create `src/hindsight/parallel-universe-bridge.ts` with ParallelUniverseBridge class
  ...
  - [ ]* 11.3 Write property test for namespace isolation
  ...
  - [ ]* 11.4 Write property test for branch merge reconciliation
  ...
  - [ ]* 11.5 Write property test for cross-agent retrieval
...
- [ ] 13. Implement ConvexAdapter and schema extension
  - [ ] 13.1 Add Hindsight tables to `convex/schema.ts`
    - Add hindsightFragments and hindsightConsolidations tables with indexes as defined in design
    - _Requirements: 2.2, 2.4_
  - [ ] 13.2 Create `convex/hindsight.ts` with Convex mutations and queries
    - Implement storeFragment, getFragment, queryFragments, searchByVector, deleteFragment mutations/queries
    - _Requirements: 2.2, 2.4_
  - [ ] 13.3 Create `src/hindsight/convex-adapter.ts` implementing StorageAdapter for Convex
    - Implement all StorageAdapter methods using the Convex client
    - Handle Convex unavailability gracefully (isAvailable check)
    - _Requirements: 2.2, 2.4_

- [ ] 14. Wire everything together and create public API
  - [ ] 14.1 Create `src/hindsight/index.ts` as the public module entry point
  ...
  - [ ] 14.2 Integrate ATLASBridge with existing KronosAtlas in `src/atlas/index.ts`
  ...
  - [ ] 14.3 Integrate TasteVaultBridge with existing TasteVault in `src/taste-vault/taste-vault.ts`
