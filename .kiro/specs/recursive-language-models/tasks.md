...
- [ ] 1. Set up RLM module structure and core types
...
- [ ] 2. Implement ContextWindow serialization
  - [ ] 2.1 Implement `serializeContextWindow()` and `deserializeContextWindow()` in `src/rlm/context-window.ts`
    - JSON serialization with version field
    - Validation on deserialization (check required fields, score ranges)
    - _Requirements: 9.1, 9.2, 9.3_

  - [ ]* 2.2 Write property test for ContextWindow round-trip
    - **Property 18: ContextWindow serialization round-trip**
    - **Validates: Requirements 9.1, 9.2, 9.3**

  - [ ]* 2.3 Write unit tests for ContextWindow edge cases
    - Empty segments array, missing fields, invalid scores
    - _Requirements: 9.1, 9.2_

- [ ] 3. Implement Reader Model adapter and model selection
  - [ ] 3.1 Extend `ExtendedModelRegistry` in `src/model-db/model-registry.ts` to include `context-compression` capability in the capability type
    - Add a pre-populated reader model entry (e.g., `ollama-qwen2.5:7b` with context-compression capability)
    - _Requirements: 2.1_

  - [ ] 3.2 Implement `selectReaderModel()` in `src/rlm/model-selection.ts`
  ...
  - [ ]* 3.3 Write property test for cheapest model auto-selection
    - **Property 6: Auto-select cheapest reader model**
    - **Validates: Requirements 2.3**

  - [ ]* 3.4 Write property test for model acceptance with capability
    - **Property 5: Model acceptance with capability**
    - **Validates: Requirements 2.2**

  - [ ] 3.5 Implement `ReaderModelAdapter` in `src/rlm/reader-adapter.ts`
...
- [ ] 5. Implement RLM Pipeline core logic
  - [ ] 5.1 Implement `RlmPipeline` class in `src/rlm/rlm-pipeline.ts`
  ...
  - [ ]* 5.2 Write property test for high-relevance segment preservation
    - **Property 3: High-relevance segment preservation**
    - **Validates: Requirements 1.4**

  - [ ]* 5.3 Write property test for fallback on reader failure
    - **Property 4: Fallback on reader failure**
    - **Validates: Requirements 1.5**

  - [ ]* 5.4 Write property test for segment relevance scores present
    - **Property 2: Segment relevance scores present**
    - **Validates: Requirements 1.2**

  - [ ]* 5.5 Write property test for runtime reconfiguration
    - **Property 7: Runtime reconfiguration**
    - **Validates: Requirements 2.4**

  - [ ]* 5.6 Write property test for pipeline activation
    - **Property 1: Pipeline activation**
    - **Validates: Requirements 1.1, 3.1**

- [ ] 6. Integrate RLM Pipeline into AgentLoop
  - [ ] 6.1 Extend `AgentLoopConfig` and `AgentLoopResult` in `src/agent-loop/agent-loop.ts`
  ...
  - [ ] 6.2 Inject RLM pipeline into `AgentLoop.callModel()` method
  ...
  - [ ]* 6.3 Write property test for token count tracking
    - **Property 8: Token count tracking in results**
    - **Validates: Requirements 3.2**

  - [ ]* 6.4 Write property test for bypass when disabled
    - **Property 9: Bypass when disabled**
    - **Validates: Requirements 3.3**

  - [ ]* 6.5 Write property test for budget uses compressed tokens
...
- [ ] 8. Implement audit and drift detection
  - [ ] 8.1 Implement audit logic in `src/rlm/audit.ts`
  ...
  - [ ]* 8.2 Write property test for audit entry completeness
    - **Property 16: Audit entry completeness**
    - **Validates: Requirements 7.1, 7.3**

  - [ ]* 8.3 Write property test for drift warning emission
    - **Property 17: Drift warning emission**
    - **Validates: Requirements 7.2**

- [ ] 9. Implement ATLAS integration
  - [ ] 9.1 Implement ATLAS storage hook in `src/rlm/atlas-integration.ts`
  ...
  - [ ]* 9.2 Write property test for ATLAS storage after compression
    - **Property 11: ATLAS storage after compression**
    - **Validates: Requirements 4.1**

  - [ ]* 9.3 Write property test for budget compliance on retrieved context
    - **Property 12: Budget compliance for retrieved context**
    - **Validates: Requirements 4.2, 4.3**

- [ ] 10. Implement CRDT collaboration integration
  - [ ] 10.1 Implement CRDT hooks in `src/rlm/crdt-integration.ts`
  ...
  - [ ]* 10.2 Write property test for CRDT compression before broadcast
    - **Property 13: CRDT compression before broadcast**
    - **Validates: Requirements 5.1**

  - [ ]* 10.3 Write property test for CRDT join summary
    - **Property 14: CRDT join summary**
    - **Validates: Requirements 5.2**

  - [ ]* 10.4 Write property test for CRDT reconciliation
    - **Property 15: CRDT reconciliation picks most recent**
    - **Validates: Requirements 5.3**

- [ ] 11. Add Convex schema and mutations for RLM config and audit logs
  - [ ] 11.1 Add `rlmConfigs` and `rlmAuditLogs` tables to `convex/schema.ts`
    - Include indexes as specified in design
    - _Requirements: 2.5, 7.4_

  - [ ] 11.2 Create `convex/rlm.ts` with queries and mutations
